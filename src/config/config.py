...
class LoRAConfig:
    ...
    modules_to_save: Optional[List[str]] = None  # 需要完全训练的模块
...
class RAGConfig:
    ...
    persist_directory: str = "./data/chroma_db"  # 如果使用ChromaDB


@dataclass
class GenerationConfig:
    """生成配置 - 适配Transformers生成接口"""
    # 基础生成参数
    ...
    max_length: Optional[int] = None  # 总长度限制
...
class DataConfig:
    ...
    # 数据格式
    input_format: str = "text"  # "text", "json", "csv"
...
class SystemConfig:
    ...
    # 硬件配置
    use_gpu: bool = True
    gpu_ids: List[int] = field(default_factory=lambda: [0])  # 支持多GPU
    num_workers: int = 4
    ...
    # 优化配置
    compile_model: bool = False  # 是否使用torch.compile (PyTorch 2.0+)
